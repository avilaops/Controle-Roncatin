use crate::{api, components, models::DashboardData};
...
impl Dashboard {
    ...
    fn render(data: &DashboardData) -> Result<(), JsValue> {
        ...
        components::set_inner_html(
            "vendasHojeValor",
            &components::format_currency(data.vendas_hoje.valor_total),
        );

        components::set_inner_html("vendasMesQtd", &data.resumo_mes.total_vendas.to_string());
        components::set_inner_html(
            "vendasMesValor",
            &components::format_currency(data.resumo_mes.valor_total),
        );

        components::set_inner_html(
            "ticketMedio",
            &components::format_currency(data.resumo_mes.ticket_medio),
        );
